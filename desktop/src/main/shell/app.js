(async () => {
  ...
  try {
    const read = window.tongs.listAssets();
    const assets = await read.result;
    ...
    status.textContent =
      `Local service ready. ${assets.length} validated extension asset(s).`;
    await new Promise((resolve) =>
      requestAnimationFrame(() => requestAnimationFrame(resolve)),
    );
    document.documentElement.dataset.tongsProbe = JSON.stringify(probe);
  } catch {
    status.textContent = "The local Tongs service is unavailable.";
  }
})();
