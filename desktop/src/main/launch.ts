...
export function validateLaunchConfig(value: DesktopLaunchConfig): DesktopLaunchConfig {
  ...
  if (!statSync(value.pythonExecutable).isFile()) {
    throw new Error("The desktop Python interpreter must be an executable file");
  }
...
export function parseLaunchArguments(argv: readonly string[]): DesktopLaunchConfig {
  const values = new Map<string, string>();
  const allowed = new Set(["--tongs-python-executable", "--tongs-core-version", "--tongs-safe-cwd", "--tongs-smoke-report"]);
  for (let index = 0; index < argv.length; index += 1) {
    const argument = argv[index];
    if (!argument?.startsWith("--tongs-")) continue;
    const separator = argument.indexOf("=");
    const name = separator >= 0 ? argument.slice(0, separator) : argument;
    if (!allowed.has(name) || values.has(name)) throw new Error("Invalid desktop launch argument");
    if (separator >= 0) {
      values.set(name, argument.slice(separator + 1));
      continue;
    }
    const next = argv[index + 1];
    if (!next || next.startsWith("--")) throw new Error("Missing desktop launch value");
